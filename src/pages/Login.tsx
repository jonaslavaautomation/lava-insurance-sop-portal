...
// Google is the primary, one-click path. Password sign-up exists
// alongside it for VAs who don't have a Google account - it's deliberately
// NOT a way to become admin: role is decided entirely server-side, and a
// password account can only ever land as va_student, no matter what email
// it uses (see handle_new_user() in supabase/sql-editor/00_full_schema.sql
// - admin is gated on the sign-up actually coming from a verified Google
// identity, not just a matching email).
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#121723]/80 rounded-2xl border border-white/[0.08] p-8">
          ...
          <div className="flex items-center gap-3 my-6">
            ...
            <span className="text-xs text-slate-600">No Google account?</span>
            <div className="flex-1 h-px bg-white/[0.08]" />
          </div>

          <div className="flex gap-2 mb-6 p-1 bg-white/[0.04] rounded-xl">
            <button
              onClick={() => { setMode('login'); setError(null); setConfirmEmailSent(false); }}
            ...
            <button
              onClick={() => { setMode('signup'); setError(null); setConfirmEmailSent(false); }}
              className={`flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-all ${
          ...
          {confirmEmailSent ? (
            <div className="flex items-start gap-2.5 text-sm text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-4 py-3.5">
              <CheckCircle2 className="w-4 h-4 flex-shrink-0 mt-0.5" />
              <span>Account created — check {email} for a confirmation link before signing in.</span>
            </div>
          ) : (
        ...
        <p className="text-center text-xs text-slate-600 mt-6">
          VAs and students: sign in with Google if you have an account, or create a password account above if you don't.
        </p>
      </div>
    </div>
  );
